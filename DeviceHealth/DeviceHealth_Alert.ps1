...
function Send-AlertEmail {
    ...
    try {
        ...
        $credPath = Join-Path $PSScriptRoot "..\smtp_credential.xml"
        if (Test-Path $credPath) {
            $cred = Import-Clixml -Path $credPath
            $mailParams["Credential"] = $cred
        } else {
            Write-Log "SMTP Credential file not found. Securely store password by running: Get-Credential | Export-Clixml '$credPath'" "WARN"
        }
